...
export default async function SocialPage() {
  ...
  try {
    [snapshot, settings] = await Promise.all([getLatestSnapshot(), getXMonitorSettings()]);
  } catch (error) {
  ...
  return (
    <main className="dashboard-page">
      ...
      <div className="page-shell dashboard-content">
        ...
        <XCollectionPanel initialLookbackDays={settings.lookbackDays} initialPerAccountPostLimit={settings.perAccountPostLimit} initialTotalPostLimit={settings.totalPostLimit} accountCount={settings.usernames.length} />
        {!settings.usernames.length ? <aside className="account-required"><strong>먼저 모니터링 계정을 등록하세요.</strong><Link href="/settings">계정 설정으로 이동 →</Link></aside> : null}
