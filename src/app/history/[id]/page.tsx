    ...
    <main className="dashboard-page">
      ...
      <div className="page-shell dashboard-content">
        ...
        {!snapshot ? <section className="empty-state social-empty"><div className="empty-orbit"><span>0</span></div><p className="kicker">NOT FOUND</p><h2>해당 갱신 기록이 없습니다.</h2><p>보관 개수 제한으로 정리됐거나 존재하지 않는 주소입니다.</p></section> : (
          <>
            <section className="history-detail-head"><div><p className="kicker">SAVED SNAPSHOT</p><h1>{source === "macro" ? "매크로 갱신" : source === "market" ? "시장지수 갱신" : source === "social" ? "X 수집·분석" : source === "all" ? "전체 데이터 갱신" : "이전 갱신 결과"}</h1><p>{formatDateTime(snapshot.createdAt)}에 저장된 결과입니다. 조회만으로 API 요금은 발생하지 않습니다.</p></div><span className={`history-source large ${source ?? "unknown"}`}>{source === "macro" ? "MACRO" : source === "market" ? "MARKET" : source === "social" ? "X" : source === "all" ? "ALL" : "이전"}</span></section>
            {(source === "macro" || source === "all" || source === null) && snapshot.payload.macro.length ? <MacroResults series={snapshot.payload.macro} warnings={snapshot.payload.macroWarnings} /> : null}
            {(source === "market" || source === "all" || source === null) && snapshot.payload.market?.series.length ? <MarketResults market={snapshot.payload.market} /> : null}
            {(source === "social" || source === "all" || source === null) && (snapshot.payload.social.posts.length || snapshot.payload.social.companies.length) ? <SocialResults social={snapshot.payload.social} expanded /> : null}
          </>
        )}
      </div>
      <SiteFooter />
