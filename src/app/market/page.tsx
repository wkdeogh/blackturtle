  ...
  return (
    <main className="dashboard-page">
      ...
      <div className="page-shell dashboard-content">
        ...
        {missing.length || databaseError ? <aside className="setup-alert" role="status"><div><span className="alert-dot" /><strong>{databaseError ? "데이터베이스 확인이 필요합니다" : "시장 데이터 설정이 남아 있습니다"}</strong></div><p>{databaseError || `미설정 환경 변수: ${missing.join(", ")}`}</p><p className="setup-help">Twelve Data 무료 API 키와 최신 Supabase migration을 등록한 뒤 재배포하세요.</p></aside> : null}

        {!market?.series.length ? (
          <section className="empty-state"><div className="empty-orbit"><span>0</span></div><p className="kicker">NO MARKET DATA YET</p><h2>아직 저장된 시장 데이터가 없습니다.</h2><p>{getSupabaseAdmin() ? "첫 시장지수 갱신을 실행하면 최근 3년 일간 가격을 저장합니다. 무료 API 호출 한도 때문에 완료까지 약 1분 걸립니다." : "Supabase 연결과 환경 변수를 완료한 뒤 첫 갱신을 실행하세요."}</p><RefreshButton source="market" initialRun={latestRun} /></section>
