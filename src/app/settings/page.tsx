  ...
  return (
    <main className="dashboard-page">
      ...
      <div className="page-shell dashboard-content settings-page-content">
        ...
        <XAccountSettings
          key={`${settings.source}:${settings.accounts.map((account) => `${account.username}:${account.enabled ? "1" : "0"}`).join("|")}`}
          initialAccounts={settings.accounts}
          source={settings.source}
          statusReady={settings.accountStatusReady}
        />
      </div>
      <SiteFooter />
    </main>
