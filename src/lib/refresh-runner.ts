...
export async function collectRefreshSnapshot(source: RefreshSource): Promise<{ snapshot: DashboardSnapshot; metrics: Record<string, unknown> }> {
  ...
  return { snapshot: {
    ...
    socialAccountCollectedAt: source === "social" ? now : previous?.payload.socialAccountCollectedAt,
    socialAccountAnalyzedAt: source === "social" ? now : previous?.payload.socialAccountAnalyzedAt,
