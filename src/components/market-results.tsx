...
function MarketCard({ series }: { series: MarketSeries }) {
  const drawdown = Math.min(series.drawdownPercent, 0);
  return (
    <article className="market-card">
      <header className="market-card-head">
        <div><span className="data-tag">{instrumentLabel(series)} · {series.symbol}</span><h3>{series.label}</h3></div>
      ...
      <div className="market-stat-row">
        <div><span>전일 대비</span><b className={(series.changePercent ?? 0) >= 0 ? "up" : "down"}>{series.changePercent === null ? "-" : `${series.changePercent >= 0 ? "+" : ""}${series.changePercent.toFixed(2)}%`}</b></div>
      ...
      <footer className="market-card-foot">
        ...
        {series.benchmark ? <span>{series.benchmark}</span> : <span>Twelve Data</span>}
      </footer>
    </article>
  );
}
  ...
  return (
    <>
      ...
      <section className="section-block market-section">
        <div className="section-title"><div><p className="kicker">MARKET PRICES</p><h2>주요 시장</h2></div><p>낙폭은 최근 3년 일간 종가 고점 기준</p></div>
        <div className="market-grid">{market.series.map((series) => <MarketCard series={series} key={series.id} />)}</div>
      </section>
