...
export function SocialSubnav() {
  ...
  return <div className="social-subnav-wrap">
    <nav className="section-subnav social-subnav" aria-label="X 모니터링 유형">
      {ITEMS.map((item) => <Link className={(item.exact ? pathname === item.href : pathname.startsWith(item.href)) ? "active" : ""} href={item.href} key={item.href}>{item.label}</Link>)}
    </nav>
    <nav className="social-utility-nav" aria-label="X 모니터링 보조 메뉴">
      {UTILITIES.map((item) => <Link className={pathname.startsWith(item.href) ? "active" : ""} href={item.href} key={item.href}>{item.label}</Link>)}
    </nav>
  </div>;
}
