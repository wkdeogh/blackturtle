...
function LegacyComprehensiveReport({ stored, currentSnapshotId }: { stored: StoredComprehensiveAnalysis; currentSnapshotId: string | null }) {
  ...
  return <article className="analysis-report">
    ...
    <section className="analysis-section">
      ...
      <div className="analysis-insight-list stagger-grid">{report.keyInsights.slice(0, 3).map((insight, index) => <article key={`${index}-${insight.title}`}>
    ...
    <DeferredRender className="deferred-report-block" minHeight={520}><section className="analysis-section analysis-two-column">
    ...
    </section></DeferredRender>

    <DeferredRender className="deferred-report-block" minHeight={330}><section className="analysis-section"><div className="section-title"><div><p className="kicker">04 · SCENARIOS</p><h2>조건별 시나리오</h2></div><p>확률 예측이 아닌 확인 조건</p></div><div className="analysis-scenarios stagger-grid">{report.scenarios.slice(0, 3).map((scenario) => <article key={scenario.name}><span>{shortText(scenario.name, 20)}</span><ReportList items={scenario.conditions} /><p><b>예상 영향</b>{shortText(scenario.marketImpact, 110)}</p><p><b>대응 관점</b>{shortText(scenario.response, 90)}</p></article>)}</div></section></DeferredRender>

    <DeferredRender className="deferred-report-block" minHeight={390}><section className="analysis-section"><div className="section-title"><div><p className="kicker">05 · WATCHLIST</p><h2>다음 확인 항목</h2></div></div><div className="analysis-watchlist stagger-grid">{report.watchlist.slice(0, 4).map((item, index) => <article key={`${index}-${item.item}`}><span>{index + 1}</span><div><h3>{shortText(item.item, 60)}</h3><p>{shortText(item.currentContext, 100)}</p><small>{shortText(item.whyItMatters, 100)}</small><b>확인 기준 · {shortText(item.trigger, 100)}</b></div></article>)}</div></section></DeferredRender>

    <DeferredRender className="deferred-report-block" minHeight={250}><section className="analysis-bottom-line"><p className="kicker">BOTTOM LINE</p><h2>{shortText(report.bottomLine, 320)}</h2>{report.dataCaveats.length ? <details><summary>데이터 한계 {Math.min(report.dataCaveats.length, 3)}건</summary><ReportList items={report.dataCaveats.slice(0, 3)} /></details> : null}<small>대시보드에 저장된 데이터만 사용한 AI 분석이며 투자 조언이 아닙니다.</small></section></DeferredRender>
  </article>;
}

function MarkdownComprehensiveReport({ stored, currentSnapshotId }: { stored: StoredComprehensiveAnalysis; currentSnapshotId: string | null }) {
  const report = stored.report;
  if (report.version !== 2) return null;
  const stale = Boolean(currentSnapshotId && report.sourceSnapshotId !== currentSnapshotId);
  return <article className="analysis-report analysis-markdown-report">
    {stale ? <aside className="analysis-stale">이 리포트 작성 후 대시보드 데이터가 갱신됐습니다. 새 데이터 반영이 필요하면 다시 분석하세요.</aside> : null}
    <header className="analysis-markdown-meta">
      <div><p className="kicker">COMPREHENSIVE REPORT</p><h2>종합분석 리포트</h2><p>자유형식 Markdown 보고서</p></div>
      <dl><div><dt>작성</dt><dd>{formatDateTime(report.generatedAt)}</dd></div><div><dt>모델</dt><dd>{report.model}</dd></div><div><dt>예상 입력</dt><dd>약 {number(report.estimatedInputTokens)} tokens</dd></div></dl>
    </header>
    <div className="analysis-markdown-body">
      <ReactMarkdown
        ...
        skipHtml
        components={{
          a: ({ children, node, ...props }) => { void node; return <a {...props} target="_blank" rel="noreferrer noopener">{children}</a>; },
          img: ({ alt, node }) => { void node; return <span className="analysis-markdown-image-note">[이미지 생략{alt ? `: ${alt}` : ""}]</span>; },
          table: ({ children, node, ...props }) => { void node; return <div className="analysis-markdown-table"><table {...props}>{children}</table></div>; },
        }}
      >{report.markdown}</ReactMarkdown>
    </div>
    <footer className="analysis-markdown-footer">대시보드에 저장된 데이터만 사용한 AI 분석이며 투자 조언이 아닙니다.</footer>
  </article>;
}

function ComprehensiveReport(props: { stored: StoredComprehensiveAnalysis; currentSnapshotId: string | null }) {
  return props.stored.report.version === 2 ? <MarkdownComprehensiveReport {...props} /> : <LegacyComprehensiveReport {...props} />;
}
...
export function ComprehensiveAnalysisPanel({ initialRun, initialReport, currentSnapshotId, migrationReady, hasData, analysisModel }: {
  ...
  return <>
    ...
    {preview ? <div className="analysis-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !starting) setPreview(null); }}><section className="analysis-modal" role="dialog" aria-modal="true" aria-labelledby="analysis-confirm-title"><p className="kicker">COST CONFIRMATION</p><h2 id="analysis-confirm-title">종합분석을 실행할까요?</h2><p>현재 저장 데이터를 직렬화한 <b>예상 입력 토큰</b>입니다. 실제 사용량과 요금은 모델 토크나이저 및 reasoning·응답 길이에 따라 달라집니다.</p><div className="analysis-token-count"><span>약</span><strong>{number(preview.estimatedInputTokens)}</strong><small>입력 tokens</small></div><dl><div><dt>모델</dt><dd>{preview.model} · reasoning medium</dd></div><div><dt>포함 데이터</dt><dd>매크로 {preview.dataCounts.macro} · 시장 {preview.dataCounts.market} · 시총 {preview.dataCounts.marketCaps} · 포트폴리오 {preview.dataCounts.portfolio} · 일정/공시 {preview.dataCounts.events} · X {preview.dataCounts.posts}</dd></div><div><dt>출력 상한</dt><dd>reasoning 포함 최대 {number(preview.maxOutputTokens)} tokens</dd></div></dl><aside>확인을 누를 때만 OpenAI 유료 호출이 시작됩니다. 작업 중 페이지를 나가도 계속 진행되며 자동 재호출은 하지 않습니다.</aside><div className="analysis-modal-actions"><button className="secondary-button" type="button" onClick={() => setPreview(null)} disabled={starting}>취소</button><button className="combined-button" type="button" onClick={() => void startAnalysis()} disabled={starting}>{starting ? "시작 중…" : "확인 후 분석 시작"}</button></div></section></div> : null}

    {manualPrompt ? <div className="analysis-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setManualPrompt(null); }}><section className="analysis-modal analysis-manual-modal" role="dialog" aria-modal="true" aria-labelledby="manual-prompt-title"><p className="kicker">MANUAL AI PROMPT</p><h2 id="manual-prompt-title">구독 중인 AI에 입력하세요</h2><p>원시 차트 전체 대신 현재값·기간별 변화·고점 대비 낙폭과 X 집계·대표 근거만 압축해 넣었습니다. AI가 돌려준 Markdown 결과 전체를 복사하세요.</p><div className="analysis-token-count compact"><span>예상 입력</span><strong>{number(manualPrompt.estimatedInputTokens)}</strong><small>tokens</small></div><label htmlFor="manual-analysis-prompt">종합분석 프롬프트</label><textarea id="manual-analysis-prompt" readOnly value={manualPrompt.prompt} />{error ? <p className="analysis-modal-error" role="alert">{error}</p> : null}<aside>응답 첫 줄의 원본 데이터 식별 줄을 함께 복사하세요. 저장할 때 제거되며 결과와 원본 데이터를 정확히 연결하는 데만 사용합니다.</aside><div className="analysis-modal-actions"><button className="secondary-button" type="button" onClick={() => setManualPrompt(null)}>닫기</button><button className="combined-button" type="button" onClick={() => void copyManualPrompt()}>프롬프트 다시 복사</button></div></section></div> : null}

    {manualResultOpen ? <div className="analysis-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !manualImporting) { setError(""); setManualResultOpen(false); } }}><section className="analysis-modal analysis-manual-modal" role="dialog" aria-modal="true" aria-labelledby="manual-result-title"><p className="kicker">IMPORT AI RESULT</p><h2 id="manual-result-title">분석 결과 입력</h2><p>AI가 반환한 Markdown 전체를 붙여넣으면 원본 데이터 연결을 확인한 뒤 종합분석 리포트를 업데이트합니다.</p><label htmlFor="manual-source-model">사용한 AI 또는 모델</label><input id="manual-source-model" type="text" maxLength={70} value={manualSourceModel} onChange={(event) => setManualSourceModel(event.target.value)} placeholder="예: ChatGPT GPT-5.6 Sol" /><label htmlFor="manual-analysis-result">Markdown 분석 결과</label><textarea id="manual-analysis-result" value={manualResult} onChange={(event) => setManualResult(event.target.value)} placeholder={'BLACKTURTLE-SOURCE-SNAPSHOT-ID: ...\n# 시장 분석\n\n자유형식 보고서'} />{error ? <p className="analysis-modal-error" role="alert">{error}</p> : null}<aside>이 저장 과정에서는 LLM API를 호출하지 않습니다. 원본 식별 줄은 저장 전에 제거되어 실제 보고서에는 표시되지 않습니다.</aside><div className="analysis-modal-actions"><button className="secondary-button" type="button" onClick={() => { setError(""); setManualResultOpen(false); }} disabled={manualImporting}>취소</button><button className="combined-button" type="button" onClick={() => void importManualResult()} disabled={manualImporting || !manualResult.trim()}>{manualImporting ? "검사·저장 중…" : "결과로 리포트 업데이트"}</button></div></section></div> : null}
  </>;
}
