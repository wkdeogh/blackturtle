...
export function GlobalRefreshIndicator() {
  ...
  return <span className="global-refresh-status"><i aria-hidden="true" />{run.source === "macro" ? "매크로" : run.source === "market" ? "시장" : run.source === "social" ? "X" : run.source === "all" ? "전체" : "데이터"} 갱신 중</span>;
}
